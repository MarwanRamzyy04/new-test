...
// FIX: now returns user so frontend can update auth state after token refresh
...
// FIX: no longer sends token/refreshToken in body for web — cookies only
// Sending both was double-exposing the refresh token unnecessarily
...
// Mobile: body tokens intentionally included — mobile clients store tokens themselves
// No cookies set here because mobile apps don't use HttpOnly cookies
...
// FIX: returns full user shape so frontend can hydrate auth state immediately after login
...
// FIX: added missing success: true flag, consistent data shape
...
exports.requestEmailUpdate = catchAsync(async (req, res, next) => {
  ...
  if (!newEmail) return next(new AppError('newEmail is required', 400));
